...
export class DocumentosComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.lenguajeService.idiomaSubject.subscribe(data => {
      ...
      this.lenguajeService.getTranslationTexts(data).subscribe(texts => {
          this.documentosInfo = texts;
          this.obtenerCategoriasIdioma();
        },
        () => {
          this.toastr.error(this.documentosInfo?.mensajes ? this.documentosInfo.mensajes.MENSAJE_ERROR : Constants.MENSAJE_ERROR);
        });
    });
    ...
    this.formFiltro.get('pais').valueChanges.subscribe(data => {
      ...
      this.getDocumentos(paisFil, this.pagina, categoriaFil, this.registros);
    });
    ...
    this.getCantidadDocumentos();
    this.getDocumentos('', this.pagina, '', this.registros);
  ...
  private registrosPantallaGrande(): void {
    ...
    this.configPaginador.itemsPerPage = Number(this.registros);
  }

  getDocumentos(pais: string, pagina: string, categoria: string, registros: string): void {
    ...
    this.genericoService.get(url).subscribe(res => {
      if (res.message === '200') {
        ...
        this.toastr.info(this.documentosInfo?.mensajes ? this.documentosInfo.mensajes.MENSAJE_NO_DATA : Constants.MENSAJE_NO_DATA);
      } else {
        this.toastr.error(this.documentosInfo?.mensajes ? this.documentosInfo.mensajes.MENSAJE_ERROR : Constants.MENSAJE_ERROR);
      }
    }, error => {
